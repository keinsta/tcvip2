...
const AdminUserWithdrawalHistory = ({ userId, setFirstThreeWithdrawals }) => {
  ...
  const fetchWithdrawals = async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get("/admin/get-user-withdrawal-report", {
        params: { userId, filter, page },
      });
      //   console.log(res.data);
      setData(res.data);
      setFirstThreeWithdrawals(res.data.lastThreeCompletedWithdrawals);
    } catch (err) {
  ...
  return (
    <div className="">
      <h2 className="text-2xl font-semibold mb-4">User Withdrawal Report</h2>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="bg-gray-800 p-4 rounded-xl shadow-md">
              <div className="flex items-center gap-3 text-white">
                ...
                <p className="text-lg font-semibold">
                  ₹ {data?.completedWithdrawalAmount}
                </p>
              </div>
              <p className="text-sm text-gray-200 mt-1">
                Total Completed Withdraw Amount
              </p>
            </div>
            <div className="bg-gray-800 p-4 rounded-xl shadow-md">
              <div className="flex items-center gap-3 text-white">
            ...
            <div className="bg-gray-800 p-4 rounded-xl shadow-md">
              <p className="font-semibold mb-2">Last 3 Withdrawals</p>
              <ul className="text-sm text-gray600 space-y-1">
                {data.lastThreeCompletedWithdrawals.map((tx) => (
                  <li key={tx._id}>
        ...
        <div className="flex flex-col items-center py-12 text-gray-500">
          ...
          <p>No Withdrawal history found for this period.</p>
        </div>
      )}
    </div>
  );
