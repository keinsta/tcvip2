...
const getTRXBlockHash = async () => {
  try {
    const response = await axios.get(
      "https://api.tronscan.org/api/block/latest"
    );
    // console.log("TRX Block Hash Response:", response.data);

    if (response.data && response.data.hash && response.data.number) {
      return {
        hash: response.data.hash,
        sequentialNumber: response.data.number,
      };
    } else {
      console.error("TRX Block Hash not found in the response.");
      return null;
    }
  } catch (error) {
    console.error("Error fetching TRX block hash:", error);
    return null;
  }
};
