...
router.get("/history/:mode", async (req, res) => {
  ...
  try {
    const history = await TrxWinGoGame.find({ mode })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .select("-bets");

    const totalRecords = await TrxWinGoGame.countDocuments({ mode });
...
router.get("/my-history/:userId", async (req, res) => {
  try {
    ...
    const [history, totalResult] = await Promise.all([
      TrxWinGoGame.aggregate(pipeline),
      TrxWinGoGame.aggregate(totalCountPipeline),
    ]);
