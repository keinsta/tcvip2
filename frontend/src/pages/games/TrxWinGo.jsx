...
const winGoSocket = io(`${API_BASE_URL}/game/trxwingo`, {
  transports: ["websocket"],
  autoConnect: false, // ❌ Prevent auto-reconnecting on every mode change
});

const numbers = Array.from({ length: 10 }, (_, i) => i);
const gameModes = [{ label: "Win Go 1min", mode: "1min", time: 60 }];
...
const TrxWinGoGame = () => {
  ...
  const fetchGameHistory = async (page) => {
    try {
      const response = await axiosInstance(
        `/game/trxwingo/history/${selectedMode}?page=${page}`
      );
  ...
  useEffect(() => {
    ...
    winGoSocket.on("currentBetRound", (data) => {
      // console.log("currentBetRound", data);
  ...
  useEffect(() => {
    if (countdown === 1) {
      ...
      if (betResult.gameResult?.drawnNumber) {
        ...
        if (betResult.winningPrice !== 0) {
          // console.log("abdc");
          handleUpdateUserBalance(betResult?.winningPrice);
          handleRefreshBalance();
        }
      }
      ...
      <div className="w-full h-[54px] bg-gradient-yellow-headers flex items-center justify-between px-4 shadow-md text-white">
        <div className="flex justify-center">
          <ArrowLeft
            ...
            onClick={() => navigate(-1)}
          />
          <span className="text-lg font-semibold">🎲Trx Win Go</span>
        </div>
        <button
          ...
          >
            ...
            <div className="grid grid-cols-5 gap-5">
              {numbers.map((num) => {
                ...
                let baseClasses =
                  "w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 flex items-center justify-center text-lg md:text-xl lg:text-2xl font-extrabold rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 active:scale-95";
                ...
                if (isSelected) {
                  ...
                  shadowEffect =
                    "shadow-[inset_4px_4px_8px_rgba(255,255,255,0.5),_4px_4px_10px_rgba(0,0,0,0.4)]";
                } else if (num === 0 || num === 5) {
                  colorClasses =
                    "bg-gradient-to-br from-green-400 via-purple-500 to-red-500 text-white border border-purple-700";
                  shadowEffect =
                    "shadow-[inset_2px_2px_4px_rgba(255,255,255,0.2),_4px_4px_8px_rgba(0,0,0,0.3)]";
                } else if (num % 2 === 0) {
                  colorClasses =
                    "bg-red-500 text-white border border-red-700 hover:bg-red-600";
                  shadowEffect =
                    "shadow-[inset_2px_2px_4px_rgba(255,255,255,0.2),_4px_4px_8px_rgba(0,0,0,0.3)]";
                } else {
                  colorClasses =
                    "bg-green-500 text-white border border-green-700 hover:bg-green-600";
                  shadowEffect =
                    "shadow-[inset_2px_2px_4px_rgba(255,255,255,0.2),_4px_4px_8px_rgba(0,0,0,0.3)]";
                }
                ...
                return (
                  <motion.button
                    ...
                    className={`${baseClasses} ${colorClasses} ${shadowEffect}`}
                  >
                    {num}
                  </motion.button>
                );
